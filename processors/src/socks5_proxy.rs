...
pub async fn server_handshake(mut stream: RWPair, conn: &mut Connection) -> Result<RWPair> {
  // Read version
  let version = stream.read_u8().await?;
  if version != v5::VERSION {
    return Err(anyhow!("Incorrect version"));
  }
  ...
  conn.dest_addr = Some(SocketAddress::new(address, port));
