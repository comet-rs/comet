...
impl SsrClientAuthProcessor {
    ...
    pub fn new_param(protocol: SsrClientAuthType, param: &str) -> Result<Self> {
        Ok(if param.contains(':') {
            ...
            let user_id = param_split.next().unwrap().parse()?;
...
impl<RW> AuthAes128ClientStream<RW> {
    ...
    fn pack_auth_data(&self, buf: &[u8]) -> Result<Bytes> {
        ...
        let mut part12_hmac_key =
            BytesMut::with_capacity(self.user_key.len() + self.write_iv.len());
            ...
            match &mut me.write_state {
                WriteState::PreparingData => {
                    let chunks = if !me.header_sent {
                        ...
                        let mut chunks =
                            me.pack_data(&buf[divide_pos..]).map_err(io_other_error)?;
            ...
            match &mut me.read_state {
                ...
                ReadState::Data { payload_len } => {
                    if !me.read_buf.is_empty() && *payload_len > 0 {
                        // First, flush buffer
                        let n =
                            cmp::min(buf.remaining(), cmp::min(me.read_buf.len(), *payload_len));
